...
# =========================================================
# Research-HTML-Exports für Master-Index                  ===
# =========================================================
...
save_simple_html_table <- function(tbl, file, title) {
  if (!requireNamespace("knitr", quietly = TRUE)) {
    stop("Package 'knitr' is required to save HTML tables.", call. = FALSE)
  }
  ...
  table_html <- paste0(
    capture.output(
      print(knitr::kable(tbl, format = "html", escape = TRUE))
    ),
    collapse = "\n"
  )
  ...
  html <- c(
    ...
    "<head>",
    "  <meta charset='utf-8'>",
    paste0("  <title>", html_escape_simple(title), "</title>"),
    "  <style>",
    ...
    "</head>",
    "<body>",
    paste0("  <h1>", html_escape_simple(title), "</h1>"),
    table_html,
    "</body>",
    "</html>"
  )
...
research_table_dir_01 <- file.path(
  project_root,
  "data_output",
  "documentation",
  "gt_tables"
...
dir.create(research_table_dir_01, recursive = TRUE, showWarnings = FALSE)

research_tables_01 <- list(
  list(
    object = "cleaning_summary_all",
    filename = "01_cleaning_summary_all.html",
    title = "Cleaning Summary"
  ),
  list(
    object = "ip_duplicate_summary_all",
    filename = "02_ip_duplicate_summary_all.html",
    title = "Duplicate IP Summary"
  ),
  list(
    object = "pre_followup_summary",
    filename = "03_pre_followup_summary.html",
    title = "Pre Follow-up Summary"
  ),
  list(
    object = "match_summary",
    filename = "04_match_summary.html",
    title = "Email Match Summary"
  ),
  list(
    object = "match_summary_valid_only",
    filename = "05_match_summary_valid_only.html",
    title = "Email Match Summary Valid Only"
  )
)

invisible(
  lapply(research_tables_01, function(x) {
    if (exists(x$object, inherits = TRUE)) {
      tbl <- get(x$object, inherits = TRUE)

      if (is.data.frame(tbl)) {
        save_simple_html_table(
          tbl = tbl,
          file = file.path(research_table_dir_01, x$filename),
          title = x$title
        )

        message("Research HTML table saved: ", file.path(research_table_dir_01, x$filename))
      } else {
        message("Skipped research export because object is not a data frame: ", x$object)
      }
    } else {
      message("Skipped research export because object does not exist: ", x$object)
    }
  })
)
